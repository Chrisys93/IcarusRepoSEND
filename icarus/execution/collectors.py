...
@register_data_collector('StoragePlacement')
class StoragePlacementCollector(DataCollector):
    """Data collector measuring latency, i.e. the delay taken to delivery a
    content.

    TODO: The logical flow of feedback-to-action should be:
        content/request -> network -> collector -> onpath/whatever strategy -> (re)routing
        BUT: decision on storage placement should either be made oin collector, or another
        class/entity should be created, specifically for management decisions, to be placed
        (in the above described process) between collector and "onpath/whatever strategy".
    ...
    def __init__(self, view, cdf=False):
        ...
        Parameters
        ----------
        request_labels : dictionary with certain amount of dictionary entries
            Each entry of the dictionary represents an entry for content requested, with that dictionary label
        storage_labels : dictionary with certain amount of dictionary entries
            Each entry of the dictionary represents an entry for stored content, with that dictionary label
        view : NetworkView
